...
static void fill_event_from_regs(pid_t pid, int entering,
  ...
  /* numero da syscall: orig_rax preserva o numero mesmo apos a saida */
  ev->syscall_no = (long)regs->orig_rax;

  /* argumentos capturados na entrada (rdi, rsi, rdx, r10, r8, r9) */
  ev->args[0] = regs->rdi;
  ev->args[1] = regs->rsi;
  ev->args[2] = regs->rdx;
  ev->args[3] = regs->r10;
  ...
  /* valor de retorno: valido apenas na saida */
...
static int wait_for_syscall_stop(pid_t child, int *status) {
  ...
  do {
    ...
    if (WIFSTOPPED(*status)) {
      ...
      /* "Uma parada comum por SIGTRAP não deve ser reenviada ao filho. Outros
       * sinais podem ser repassados no próximo PTRACE_SYSCALL."
       */
...
int trace_program(char *const argv[], trace_observer_fn observer,
  ...
  while (1) {
    ...
    /* Le os registradores do processo filho via ptrace */
    if (ptrace(PTRACE_GETREGS, child, NULL, &regs) < 0) {
      perror("Erro ao ler registradores");
      return -1;
    }
